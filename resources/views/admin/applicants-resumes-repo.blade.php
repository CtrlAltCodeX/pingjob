...
@section('content')
    ...
    @if ($employer && $employer->premium_jobs_balance)
        @if ($applications->count() > 0)
            <a href="{{ route('download_repo_resumes_zip') }}" class="btn btn-success m-2">Download listed resumes</a>
        @endif
        @if ($employer->is_admin())
            <a href="{{ route('add_to_resumes_repo') }}" class="btn btn-success m-2">Add resumes</a>
        @endif
        <div class="col-md-12">
            @if ($applications->count() > 0)
                ...
                <div class="row">
                    <div class="badge-primary" style="width: 100%; text-align: center; font-size: 30px; font-weight: 600;">
                        <span class="text-success px-2"><i class="la la-bitbucket"></i></span>
                        {{ __('app.resumes') }}
                    </div>
                    @foreach ($applications as $application)
                        <div class="col-12 col-lg-6 d-flex p-1 px-3"
                            ...
                            <div style="width: 40%;" class="d-flex flex-column align-items-center justify-content-around">
                                ...
                                <p class="text-success"><i class="la la-download"></i> <a
                                        href="{{ route('download_resume', $application->resume) }}"
                                        class="text-success">Download Resume </a>
                                </p>
                            </div>
                            <div style="width: 60%">
                                <p class="text-info"><i class="la la-user"></i> {{ $application->name }}</p>
                                <p class="text-info"><i class="la la-clock-o"></i>
                                ...
                                <p class="text-info"><i class="la la-envelope-o"></i> {{ $application->email }}</p>
                                <p class="text-info mb-0"><i class="la la-phone-square"></i>
                                    {{ $application->phone_number }}</p>
                                <p class="text-primary mb-0"><i class="la la-bar-chart"></i>
                                    {{ $application->category_name }} - {{ $application->resume_score }}</p>
                                {{-- @php
