...
@section('content')
    <div class="row">
        <div class="col-md-10">
            ...
            <form method="post" action="{{ route('employer_profile_update') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group row {{ $errors->has('company') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control {{ e_form_invalid_class('company', $errors) }}"
                            id="company" value="{{ $user->company }}" name="company" placeholder="@lang('app.company')"
                            disabled="disabled">
                    ...
                    <div class="col-sm-8">
                        <select class="form-control {{ e_form_invalid_class('company_size', $errors) }}" name="company_size"
                            ...
                            @foreach (company_size() as $size => $size_name)
                                <option value="{{ $size }}" {{ selected($size, $user->company_size) }}>
                                    {{ $size_name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row">
                    <label for="address_2" class="col-sm-4 col-form-label">Pin Code</label>
                    <div class="col-sm-8">
                        <input id="zip_code" required type="text" class="form-control" name="zip_code"
                            value="{{ $user->zip_code }}">
