                            ...
                            <tbody>
                                ...
                                @if($search_clients->isNotEmpty())
                                    @foreach($search_clients as $search_client)
                                        <tr class="text-center">
                                            <td><a href="{{ route('clients_details', $search_client->id) }}">{{ $search_client->company }}</a></td>
                                            <td>{{ $search_client->vendor_count }}</td>
                                            <td>{{ ($search_client->review_rating > 0) ? $search_client->review_rating : 0 }}</td>
                                            <td>{{ $search_client->contact_count }}</td>
                                            <td><a href="{{route('jobs_by_employer', $search_client->company_slug)}}">{{ $search_client->job_count }}</a></td>
