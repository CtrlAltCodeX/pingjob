            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="employer_id" class="employer_id" id="employer_id" value="{{ $employer?->id }}">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="alert alert-success alert-dismissible fade show" role="alert" style="display:none;" id="alert_success">
                            ...
                            <div class="alert alert-danger alert-dismissible fade show" role="alert" style="display:none;" id="alert_danger">
                            ...
                            <div class="form-group ">
                                ...
                                <input type="text" class="form-control" id="vendor" name="vendor" value="" placeholder="Vendor name">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default" data-dismiss="modal" onclick="clearFields();">Close
                            </button>
                    ...
                    <form action="{{ route('add_contact') }}" method="post">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group ">
                                ...
                                <input type="text" class="form-control " id="first_name" required name="first_name" value="" placeholder="First Name">

                            </div>

                            <div class="form-group ">
                                <label for="name" class="control-label">@lang('app.middle_name')</label>
                                <input type="text" class="form-control " id="middle_name" name="middle_name" value="" placeholder="Middle Name">
                            ...
                            <div class="form-group ">
                                ...
                                <input type="number" class="form-control " id="primary_phone" name="primary_phone" value="" placeholder="Primary Phone">

                            </div>

                            <div class="form-group ">
                                <label for="name" class="control-label">@lang('app.cell_phone')</label>
                                <input type="number" class="form-control " id="cellphone" name="cell_phone" placeholder="Cellphone">
            ...
            <div class="col-md-8">
                ...
                <div class="reviews mb-1">
                    <h4> {{ $employer?->company }}
                        ...
                        @for ($i = 0; $i < round($review_ratings); $i++) <span><i class="checked la la-star"></i></span>
                            @endfor
                            @for ($i = 0; $i < $review_total; $i++) <span><i class="unchecked la la-star"></i></span>
                                @endfor

                                @if ($review_count != 0)
                                <a href="{{ route('user_reviews', ['job_id' => $job->id, 'employer_id' => $employer->id]) }}" class="ml-2" style="font-size: 16px; color: rgb(0, 0, 0);">{{ $review_count }}
                                    Reviews</a>
                    </h4>
                    @endif
                ...
                <p>
                    ...
                    <a href="{{ route('reviews', ['job_id' => $job->id, 'employer_id' => $employer?->id]) }}" class="btn btn-success"><i class="la la-star"></i>&nbsp;Write a Review</a>

                    <a data-toggle="modal" href="#" style="color:#fff;" data-target="#addVendorModal" class="btn btn-success"><i class="la la-plus"></i>Add Vendor&nbsp;&nbsp;</a>

                    <a data-toggle="modal" href="#" style="color:#fff;" data-target="#addContactModal" class="btn btn-success"><i class="la la-plus"></i>Add Contact&nbsp;&nbsp;</a>
                    @php
                    $currentUser = auth()->user();
                    @endphp

                    @if ($currentUser && $currentUser->premium_jobs_balance)
                    <a href="{{ route('employer_applicant_resumes_by_job_id', $job->id) }}" style="color:#fff;" class="btn btn-success"><i class="la la-eye"></i> @lang('app.resumes')
                        ({{ $count }})&nbsp;&nbsp;</a>
                    @endif
...
<div class="container">
    <div class="row">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="post" action="{{ route('comment.add') }}">
                        @csrf
                        <div class="form-group">
                            <textarea class="form-control" name="comment_body" required type="text" rows="4" placeholder="Enter your message here"></textarea>
        ...
        <div class="col-md-4">
            ...
            <div class="widget-box bg-white p-3 mb-3 box-shadow">
                ...
                @guest
                @if (count($vendors) > 2)
                <h5>( <a href="{{ route('redirect-login', ['slug' => $job->job_slug, 'page' => 'job_view']) }}">Login</a>
                    to see
                ...
                <div class="employer-job-listing mt-4">
                        ...
                        <h5><a href="#">{{ $vendor->company }}</a>
                            ...
                            @for ($i = 0; $i < round($review_ratings); $i++) <span><i class="checked la la-star"></i></span>
                                ...
                                @for ($i = 0; $i < $review_total; $i++) <span><i class="unchecked la la-star"></i></span>
                                    ...
                                    <a href="{{ route('reviews', ['employer_id' => $employer->id]) }}" class="float-right mt-3 writeReview">Write a Review</a>
                        </h5>
                        <p class="text-muted">
                            {{ $vendor->address }}</br>
                            {{ $vendor->city }}</br>
                    ...
                    @else
                    @if ($loop->index < 3) <div class="employer-job-listing-single box-shadow bg-white mb-4 p-3">
                        <h5><a href="#">{{ $vendor->company }}</a>
                            @php
                            $review_ratings = !empty($vendor->review_rating) ? $vendor->review_rating : 0.0;
                            $review_total = 5 - round($review_ratings);
                            ...
                            @for ($i = 0; $i < round($review_ratings); $i++) <span><i class="checked la la-star"></i></span>
                                ...
                                @for ($i = 0; $i < $review_total; $i++) <span><i class="unchecked la la-star"></i></span>
                                    @endfor
                                    <br />
                                    <a href="{{ route('reviews', ['employer_id' => $employer->id]) }}" class="float-right mt-2 writeReview">Write a Review</a>
                        </h5>
                        <p class="text-muted">
                            {{ $vendor->address }}</br>
                            {{ $vendor->city }}</br>
        ...
        <div class="widget-box bg-white p-3 mb-3 box-shadow">
            ...
            <p>
                ...
                : {{ Carbon\Carbon::parse($job->deadline)->format(get_option('date_format')) }} <span class="text-small text-muted">{{ Carbon\Carbon::parse($job->deadline)->diffForHumans() }}</span>
            </p>
    ...
    <button type="button" class="btn btn-success employer-follow-button"
                                                            data-employer-id="{{ $job->employer->id }}"><i class="la la-minus-circle"></i> @lang('app.unfollow') {{ $employer->company }}
            </button>
            @else
            <button type="button" class="btn btn-success employer-follow-button" data-employer-id="{{ $job->employer->id }}"><i class="la la-plus-circle"></i> @lang('app.follow') {{ $employer->company }}
            </button>
            @endif
            @endif
            </p>
        ...
        <div class="widget-box bg-white p-3 box-shadow">
            ...
            <div class="additional-job-action-box">
                ...
                <p><a href="{{ route('jobs_by_employer', $job->employer->company_slug ?? 0) }}"><i class="la la-list-ul"></i> @lang('app.check_all_job_employer') </a></p>
            ...
            <form action="{{ route('apply_job') }}" method="post" id="applyJob" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                        ...
                        <input type="text" class="form-control {{ e_form_invalid_class('name', $errors) }}" id="name" name="name" value="@if (Auth::check()) {{ $name = Auth::user()->name }} @else
    {{ old('name') }} @endif" placeholder="@lang('app.your_name')">
                        {!! e_form_error('name', $errors) !!}
                    </div>

                    <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                        <label for="email" class="control-label">@lang('app.email'):</label>
                        <input type="text" class="form-control {{ e_form_invalid_class('email', $errors) }}" id="email" name="email" value="@if (Auth::check()) {{ $email = Auth::user()->email }} @else
    {{ old('email') }} @endif" placeholder="@lang('app.email_ie')">
                        {!! e_form_error('email', $errors) !!}
                    </div>

                    <div class="form-group {{ $errors->has('phone_number') ? 'has-error' : '' }}">
                        <label for="phone_number" class="control-label">@lang('app.phone_number'):</label>
                        <input type="text" class="form-control {{ e_form_invalid_class('phone_number', $errors) }}" id="phone_number" name="phone_number" value="@if (Auth::check()) {{ $phone = Auth::user()->phone }} @else
    {{ old('phone_number') }} @endif" placeholder="@lang('app.phone_number')">
    ...
    window.addEventListener('DOMContentLoaded', function() {
        (function($) {
            $('.share').ShareLink({
                ...
                text: "{{ substr(trim(preg_replace(' / \s\ s + /', ' ', strip_tags($job->description))), 0, 160) }}", // textfor share message
